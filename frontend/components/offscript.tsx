...
import { getStreak, getHistory } from "@/lib/localProgress";
...
export function AppSidebar() {
  ...
  const [historyLevels, setHistoryLevels] = useState<number[]>([0,0,0,0,0,0,0]);
  ...
  useEffect(() => {
    const loadData = () => {
      ...
      // Calculate simple heatmap levels for the last 7 days based on activity
      ...
      history.forEach(session => {
        ...
        if (diffDays >= 0 && diffDays < 7) {
          // Just increment level up to 4
          levels[6 - diffDays] = Math.min(4, levels[6 - diffDays] + 1);
        }
      });
  ...
  return <aside className="fixed left-0 top-16 z-40 hidden h-[calc(100vh-4rem)] w-64 flex-col border-r border-border/70 bg-[#f3f4f5] p-3 md:flex">
    ...
    <div className="mb-5 rounded-xl border border-[#c7c4d7] bg-white p-4 shadow-sm"><div className="mb-2 flex items-center justify-between"><span className="text-[11px] font-semibold uppercase tracking-wider text-[#575e70]">Current streak</span><Flame size={18} className={streak > 0 ? "text-[#00885d]" : "text-[#c7c4d7]"}/></div><div className="mb-3 text-3xl font-bold">{streak} <span className="text-base font-normal text-muted-foreground">{streak === 1 ? 'day' : 'days'}</span></div><div className="grid grid-cols-7 gap-1">{historyLevels.map((level, index) => <span key={index} className={`h-5 rounded-sm ${level ? `heatmap-${level}` : "bg-[#e1e3e4]"}`}/>)}</div></div>
